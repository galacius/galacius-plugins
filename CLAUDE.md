...
`deploy-plugin-helm-local.mjs` mirror script, and the `@galacius/design-system` workspace-dependency
...
`galacius/.claude/plans/plugin-architecture-inversion.md` (host repo) for the full design rationale.
...
live in this repo but have been extracted into `packages/core/async` in the sibling `galacius` host
...
(`github.com/galacius/galacius/packages/core`); the frontend counterpart, `@galacius/core`, is
consumed the same way from npm. Both were previously wired via local `replace`/`link:` directives
into the sibling `galacius` checkout before `packages/core` was tagged/released.
...
bundle/`external` setup, the `wailsBridge.ts` fetch-retry contract, `@galacius/core`
...
- Go backend and TS frontend payload shapes must be kept in sync by hand across three places: the
  ...
  `@galacius/core`'s `createPluginBridge`, shared across plugins — only the per-endpoint payload
