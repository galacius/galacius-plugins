...
This is the official plugin repository for [Galacius](https://github.com/galacius/galacius), a
...
- A sibling checkout of the [`galacius`](https://github.com/galacius/galacius) host repo. Both
...
`@galacius/core` and `@galacius/design-system` resolve from the normal npm registry — no local
...
- **Go module** — a single root module `github.com/galacius/galacius-plugins`, plus one nested
...
`plugins/helm/go.mod` requires the tagged `github.com/galacius/galacius/packages/core` module
directly (no `go.work` shim or local `replace` directive), and
`plugins/helm/frontend/package.json` requires the published `@galacius/core` npm package (no local
...
`pnpm-workspace.yaml`'s `minimumReleaseAgeExclude` still allow-lists specific `@galacius/core` and
`@galacius/design-system` versions so pnpm doesn't flag freshly-published releases as suspiciously
...
pnpm test:helm:fe           # helm frontend tests (vitest) — requires @galacius/design-system
...
- **New frontend dependency**: check whether it needs to go in `external` in `tsup.config.ts` too —
  ...
  `react`/`react-dom`/`@tanstack/react-query`/`@galacius/design-system`/`@galacius/core` (those
...
- **gRPC contract changes** (`plugin.proto`) are made in the `galacius` host repo
