...
(`ClusterContextEvent`/`ActiveNamespacesEvent`) were extracted out of this repo into the `galacius`
...
version (currently v1.7.11); `frontend/package.json` pulls `@galacius/core` from npm the same way —
both were published 2026-08-29, replacing the earlier local `replace`/`link:` directives into the
sibling `galacius` checkout.
...
**Why:** documented as `galacius/.claude/plans/plugin-architecture-inversion.md` in the host
(`galacius`) repo — full design rationale for the HTTP-direct call path lives there. The generic
