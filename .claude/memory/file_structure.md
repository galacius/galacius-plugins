---
name: file-structure
description: "Repo layout of galacius-plugins — pnpm workspace + Go module structure, hexagonal-architecture backend/frontend directory map for the helm plugin"
metadata: 
  node_type: memory
  type: reference
...
- Single Go module `github.com/galacius/galacius-plugins` at the root, but `plugins/helm/` has its
  own nested `go.mod` (`.../galacius-plugins/plugins/helm`) since it pulls in heavy deps
...
- The reusable gRPC pub/sub sync machinery (client, backoff, generic event-route/dispatch framework,
  ...
  currently v1.7.11); the frontend counterpart is npm's `@galacius/core` (`frontend/package.json`
